  ...
  const handleFile = useCallback(async (file: File) => {
    ...
    const bgPromise = (async () => {
      try {
        ...
        // Convert to base64 for card constructor
    ...
    const recognizePromise = (async () => {
      try {
        ...
        if (data.recognized) {
          ...
          recognizedResult = data.recognized;
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="rounded-xl bg-bg-card border border-border-subtle overflow-hidden">
            ...
            <div className="aspect-square bg-bg-secondary flex items-center justify-center p-4 relative">
              {processedUrl ? (
                <img src={processedUrl} alt="Processed" className="max-h-full max-w-full object-contain" />
              ) : isProcessing ? (
        ...
        <div className="rounded-xl bg-bg-card border border-border-subtle p-6">
          ...
          {recognized ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {recognized.brand && <SpecField label="Бренд" value={recognized.brand} />}
              {recognized.viscosity && <SpecField label="Вязкость" value={recognized.viscosity} />}
              {recognized.base_type && <SpecField label="Тип" value={BASE_TYPE_LABELS[recognized.base_type] || recognized.base_type} />}
              {recognized.api_spec && <SpecField label="API" value={recognized.api_spec} />}
              {recognized.acea_spec && <SpecField label="ACEA" value={recognized.acea_spec} />}
              {recognized.approvals && <SpecField label="Допуски" value={recognized.approvals} />}
              {recognized.oem_number && <SpecField label="OEM-номер" value={recognized.oem_number} />}
              {recognized.volume && <SpecField label="Объём" value={recognized.volume} />}
            ...
            <p className="text-sm text-text-muted">
              {recognizeStatus === 'loading' ? 'Анализирую этикетку...' : 'Загрузите фото для распознавания'}
            </p>
          ) : null}
        </div>
      )}
